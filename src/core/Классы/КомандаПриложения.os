...
Процедура Запуск(Знач АргументыCLI) Экспорт
	...
	Если ОшибкаЧтения Тогда
		ОписаниеОшибкиЧтения = СтрШаблон("Ошибка чтения параметров команды: %1",
			СтрСоединить(МассивАргументовКПарсингу, " "));
		Лог.КритичнаяОшибка(ОписаниеОшибкиЧтения);
		ВывестиСправку();
		ВызватьИсключение ОписаниеОшибкиЧтения;
	КонецЕсли;
