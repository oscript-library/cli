...
Процедура ВывестиСправку() Экспорт
	...
	ОпцииРодителяКоманды = ТаблицаОпцийРодителейДляСправки();
	
	Если ОпцииРодителяКоманды.Количество() > 0 Тогда

		Сообщить(СтрШаблон(ШаблонЗаголовкаНаименования, "Наследованные опции"));
		ВывестиТаблицуСправки(ОпцииРодителяКоманды);
...
Функция ПолучитьТаблицуДанныхДляСправки()
	...
	Таблица.Колонки.Добавить("ВыводитьВПодкомандах");
...
Функция ТаблицаОпцийРодителейДляСправки()
	
	Если КомандыРодители.Количество() = 0  Тогда
		Возврат ПолучитьТаблицуДанныхДляСправки();
	КонецЕсли;

	ТаблицаОпций = ПолучитьТаблицуДанныхДляСправки();

	Для каждого РодительКоманды Из КомандыРодители Цикл
		
		ТаблицаОпцийКомандыРодителя = РодительКоманды.ПолучитьТаблицуОпций();
		
		Если ТаблицаОпцийКомандыРодителя.Количество() = 0  Тогда
			Продолжить;
		КонецЕсли;
		

		Для каждого ОпцияРодителя Из ТаблицаОпцийКомандыРодителя Цикл
			
			Если ОпцияРодителя.Обязательный
				ИЛИ ОпцияРодителя.ВыводитьВПодкомандах Тогда
				
				НоваяОпция = ТаблицаОпций.Добавить();

				ЗаполнитьЗначенияСвойств(НоваяОпция, ОпцияРодителя);
...
Функция ПолучитьТаблицуПараметров(ИндексПараметров)
	...
	Для каждого КлючЗначение Из ИндексПараметров Цикл
		...
		НоваяЗапись.Обязательный			= ПараметрСправки.ТребоватьУстановкиПользователем;
		НоваяЗапись.ВыводитьВПодкомандах	= ПараметрСправки.ВыводитьВПодкомандах;
