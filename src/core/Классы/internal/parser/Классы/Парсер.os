...
Перем ОбязательностьАргументов;
...
Процедура ПриСозданииОбъекта(Знач ТокеныСпек, Знач ПараметрыПарсера)
	...
	СкинутьОпции = Ложь;
	ОбязательностьАргументов = Истина;
...
Функция ЧтениеДалее()
	...
	ИначеЕсли НашлиТокен(ТипыТокенов.TTArg) Тогда
		...
		КонечноеСостояние = НачальноеСостояние.Т(Новый АргументыПарсера(КлассОпции, ОбязательностьАргументов), 
	...
	ИначеЕсли НашлиТокен(ТипыТокенов.TTShortOpt)
		...
		КонечноеСостояние = НачальноеСостояние.Т(Новый ОпцияПарсера(КлассОпции, ОпцииИндекс, ОбязательностьАргументов),
												 ОбработчикВыборкиПути.НовоеСостояние());
