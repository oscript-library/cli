...
// Объект-владелец метода, предоставляющего варианты автодополнения значений параметра
Перем ОбъектПоставщикаДополнения; // Произвольный
// Имя экспортного метода-функции объекта, возвращающего Массив<Строка> вариантов
Перем ИмяМетодаПоставщикаДополнения; // Строка
...
// Возвращает имя параметра (первое слово в наименовании).
//
// Возвращаемое значение:
//   Строка
//
...
// Возвращает все имена/синонимы параметра.
//
// Возвращаемое значение:
//   Массив из Строка
//
...
// Возвращает тип параметра: "опция" или "аргумент".
//
// Возвращаемое значение:
//   Строка
//
Функция ПолучитьТипПараметра() Экспорт
...
// Регистрирует поставщик вариантов автодополнения значений параметра.
// Варианты используются командой `completions` для запекания в итоговый скрипт
// автодополнения командной оболочки.
//
// Параметры:
//   ОбъектРеализации - Произвольный - объект-владелец метода-поставщика
//   ИмяМетода        - Строка       - имя экспортной функции без параметров,
//                                     возвращающей Массив<Строка> / ФиксированныйМассив
//
// Возвращаемое значение:
//   ПараметрКоманды - значение из переменной ЭтотОбъект
//
Функция ПоставщикДополнения(Знач ОбъектРеализации, Знач ИмяМетода) Экспорт

	ОбъектПоставщикаДополнения = ОбъектРеализации;
	ИмяМетодаПоставщикаДополнения = ИмяМетода;

	Возврат ЭтотОбъект;

КонецФункции

// Возвращает признак наличия поставщика вариантов автодополнения у параметра.
//
// Возвращаемое значение:
//   Булево
//
Функция ЕстьПоставщикДополнения() Экспорт

	Возврат ОбъектПоставщикаДополнения <> Неопределено И Не ПустаяСтрока(ИмяМетодаПоставщикаДополнения);

КонецФункции

// Вызывает поставщик и возвращает массив вариантов автодополнения.
// При отсутствии поставщика возвращает пустой массив.
// При исключении внутри поставщика — записывает предупреждение в лог и
// возвращает пустой массив, чтобы генерация скрипта не прерывалась.
//
// Возвращаемое значение:
//   Массив из Строка
//
Функция ПолучитьЗначенияДополнения() Экспорт

	Результат = Новый Массив();

	Если Не ЕстьПоставщикДополнения() Тогда
		Возврат Результат;
	КонецЕсли;

	Попытка
		Делегат = Делегаты.Создать(ОбъектПоставщикаДополнения, ИмяМетодаПоставщикаДополнения);
		ЗначениеПоставщика = Делегат.Исполнить();
	Исключение
		Ошибка = ИнформацияОбОшибке();
		Лог.Предупреждение(
			"Поставщик автодополнения ""%1"" завершился с ошибкой: %2",
			ИмяМетодаПоставщикаДополнения,
			ПодробноеПредставлениеОшибки(Ошибка));
		Возврат Результат;
	КонецПопытки;

	Если ТипЗнч(ЗначениеПоставщика) = Тип("Массив")
		Или ТипЗнч(ЗначениеПоставщика) = Тип("ФиксированныйМассив") Тогда
		Для Каждого Элемент Из ЗначениеПоставщика Цикл
