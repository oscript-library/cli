...
// Определяет необходимость показа опции  в справке.
// Значение "Ложь" скрывает в справке, по умолчанию "Ложь"
Перем ВыводитьВПодкомандах Экспорт;
...
Процедура ПриСозданииОбъекта(ВходящийТипПараметра,
	...
	ВыводитьВПодкомандах = Ложь;

	ТребоватьУстановкиПользователем = Ложь;
...
// Функция устанавливает признак вывода данной опции у вложенных команды
// возвращает текущий параметр команды
//
// Параметры:
//   Признак - булево - признак вывода данной опции у вложенных команды (по умолчанию Истина)
//
// Возвращаемое значение:
//   ПараметрКоманды - Объект - ссылка текущий класс "ПараметрКоманды" значение из переменной ЭтотОбъект
Функция ВыводитьВПодкомандах(Знач Признак = Истина) Экспорт

	ВыводитьВПодкомандах = Признак;
	Возврат ЭтотОбъект;
