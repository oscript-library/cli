...
Процедура Запуск(Знач АргументыCLI) Экспорт
	...
	ОшибкаЧтения = НачальноеСостояние.Прочитать(МассивАргументовКПарсингу);
...
Процедура ВывестиСправку() Экспорт
	...
	Для каждого Родитель Из КомандыРодители Цикл
		ПолныйПуть.Добавить(Родитель.Имя);
	КонецЦикла;
	...
	Сообщить(СтрШаблон("Использование: %1 %2 %3",ПутьИспользования, Спек, СуффиксВложенныхКоманды));
