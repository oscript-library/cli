...
Функция ПолучитьСписокТестов(Знач Тестирование) Экспорт
	...
	ИменаТестов.Добавить("ТестДолжен_ПроверитьВыводСправкиСОпциямиКомандыРодителя");
...
Процедура ТестДолжен_ПроверитьВыводСправкиСОпциямиКомандыРодителя() Экспорт
	...
	АргументСтрока2 = Команда.Аргумент("ARG2", "", "Тестовый простой необязательный аргумент").Обязательный();
	АргументСтрока = Команда.Аргумент("ARG", "", "Тестовый простой необязательный аргумент").ТМассивСтрок().Обязательный(Ложь);
	ОпцияПеречисление = Команда.Опция("enum e em", "yellow", "Опция перечисление").ВыводитьВПодкомандах().ТПеречисление()
						.Перечисление("yellow", "yellow", "Это желтый цвет")
						.Перечисление("red", "red" , "Это красный цвет")
						.Перечисление("blue", "blue", "Это синий цвет")
						;
	login = Команда.Опция("login", "", "Login for credential, e.g. username or email.").ТСтрока().Обязательный();
	realm = Команда.Опция("realm", "", "Realm for credential, e.g. website or WiFi AP name.").ТСтрока().ВыводитьВПодкомандах();
	noSymbol = Команда.Опция("no-symbol", false, "Exclude symbol characters in password.").Флаг().ВыводитьВПодкомандах(Ложь);
						
	Подкоманда = Команда.ДобавитьКоманду("Вложенная команда", "Тест", ЭтотОбъект);
	login = Подкоманда.Опция("login2", "", "Login for credential, e.g. username or email.").ТСтрока().Обязательный();
	
	Команда.НачалоЗапуска();
	ПодКоманда.ВывестиСправку();
