...
Процедура ТестДолжен_ПроверитьВнутреннийМенеджерПараметровОпций() Экспорт
	...
	ТестовыеСлучаи.Добавить(ТестовыйСлучай("", ";config.string;", "Ложь;Строка;0"));
	ТестовыеСлучаи.Добавить(ТестовыйСлучай("--string Строка", ";;config.int", "Ложь;Строка;42"));
	ТестовыеСлучаи.Добавить(ТестовыйСлучай("", "config.bool;config.string;", "Истина;Строка;0"));
	ТестовыеСлучаи.Добавить(ТестовыйСлучай("--int 43 --string Строка --bool", "config.bool;config.string;config.int", "Истина;Строка;43"));
...
Функция ПодготовитьТестовуюКоманду(Спек, ИмяПроцедурыОбъекта, ПутьКФайлуНастроек) 
	...
	МенеджерПараметров = Приложение.ПолучитьМенеджерПараметров();
...
ЛогКонфигор = Логирование.ПолучитьЛог("oscript.lib.configor.provider-json");
